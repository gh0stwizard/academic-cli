...
term_cb(uv_work_t* work)
{
    ...
    CURL_CHECK(
      curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, user_connect_timeout));
    CURL_CHECK(
      curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_mem_write_cb));
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*)&storage));
#ifdef _DEBUG_CURL
...
word_cb(uv_work_t* work)
{
    ...
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_URL, url));
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L));
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, user_connect_timeout));
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_mem_write_cb));
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*)&storage));
#ifdef _DEBUG_CURL
    CURL_CHECK(curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L));
#else
    ...
    do {
        ...
        if (code == CURLE_COULDNT_RESOLVE_HOST ||
            code == CURLE_OPERATION_TIMEDOUT) {
            if (retry++ < user_retries) {
                vlog(VLOG_WARN, "%s [%d:%d]: retry #%d", word, did, wid, retry);
                storage.size = 0;
                (void)nanosleep(&user_retry_ts, NULL);
            } else
                break;
