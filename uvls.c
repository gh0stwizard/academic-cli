...
uvls_init_once(void)
{
    ...
    if (uv_guess_handle(UVLS_STDOUT) == UV_TTY) {
        CHECK(uv_tty_set_mode((uv_tty_t*)uvls_out, UV_TTY_MODE_NORMAL));
    }

    if (uv_guess_handle(UVLS_STDERR) == UV_TTY) {
        CHECK(uv_tty_set_mode((uv_tty_t*)uvls_err, UV_TTY_MODE_NORMAL));
    }
...
uvls_date(const char* fmt, char date[], size_t size)
{
    ...
    for (nnum = 0, n = ts.tv_nsec; (n != 0) && (nnum < UVLS_NSEC_SIZE);
         nnum++, n /= 10L)
        ;
