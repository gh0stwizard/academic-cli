...
parse_html(const char* data, size_t size, html_data_t** out)
{
     ...
     *   <dl>
     *     <div itemscope
     * itemtype="http://webmaster.yandex.ru/vocabularies/term-def.xml"> <dt
     * [...]> WORD </dt> <dd [...]> TEXT </dd>
     *       <!-- META (not needed) //-->
     *     </div> <!-- <div itemscope [...]/>  //-->
     *   </dl>
    ...
    div_coll = myhtml_get_nodes_by_attribute_value(tree,
                                                   NULL,
                                                   NULL,
    ...
    if (div_coll && div_coll->list && div_coll->length) {
        ...
        if (dt_coll && dt_coll->list && dt_coll->length) {
            myhtml_tree_node_t* dt =
              myhtml_node_child(dt_coll->list[dt_coll->length - 1]);
        ...
        if (dd_coll && dd_coll->list && dd_coll->length) {
            myhtml_tree_node_t* ddch =
              myhtml_node_child(dd_coll->list[dd_coll->length - 1]);
...
say_tag_id(myhtml_tree_node_t* node, const char* prepend)
{
    ...
    if (tagid == MyHTML_TAG__TEXT) {
        vlog(VLOG_TRACE,
             ...
             prepend,
             tagid,
             myhtml_token_node_text(token, 0));
    } else {
