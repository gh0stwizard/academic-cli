...
convert_html(html_data_t* html, char** out)
{
    ...
    for (; el != elend; el++) {
        ...
        if (len - off < start) {
            ...
            vlog(VLOG_TRACE,
                 "copy missing fragment: len %zu off %zu start %zu",
                 len,
                 off,
                 start);
        ...
        vlog(VLOG_TRACE,
             "copy fragment: len %zu off %zu start %zu end %zu delta %zu",
             len,
             off,
             start,
             end,
...
make_mycsi(char*** out, size_t** out_sizes)
{
    ...
    SGR* sgrs[] = { reset, bold, italic, blue };
    size_t sgr_sizes[] = {
        sizeof(reset), sizeof(bold), sizeof(italic), sizeof(blue)
    };
